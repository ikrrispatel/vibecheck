...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function step(dt: number) {
      ...
      if (gradSubtractProgram.uniforms.texelSize) gl.uniform2f(gradSubtractProgram.uniforms.texelSize, velocity.texelSizeX, velocity.texelSizeY);
      if (gradSubtractProgram.uniforms.uPressure) gl.uniform1i(gradSubtractProgram.uniforms.uPressure, pressure.read.attach(0));
      if (gradSubtractProgram.uniforms.uVelocity) gl.uniform1i(gradSubtractProgram.uniforms.uVelocity, velocity.read.attach(1));
      blit(velocity.write);
      velocity.swap();
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < e.targetTouches.length; i++) {
        updatePointerMoveData(pointer,
          scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY), pointer.color);
      }
    };
    ...
    // Start the loop immediately — no waiting for first interaction
    animFrameId = requestAnimationFrame(updateFrame);
  ...
  }, []); // empty deps — component never remounts; config is captured at mount time
